...
function calcPercent(index: number): string {
  ...
  if (trackRef.value) {
    let width = (trackRef.value?.getBoundingClientRect().width - props.dotSize) * (index / (props.segments.length - 1))
...
.slider-thumb.is-dragging {
  cursor: grabbing;
  box-shadow: 0 1px 8px color-mix(in srgb, var(--slider-fill-color) 40%, transparent);
