...
async function prepareStream(roomId: number): Promise<void> {
    ...
    demuxer.onMediaInfo = (info) => {
        ...
        logInfo(
            `[LiveAudio] MediaInfo 就绪: ${info.hasAudio ? "有音频" : "无音频"}, ${info.hasVideo ? "有视频" : "无视频"}`,
        );
    };
    ...
    demuxer.onDataAvailable = (
        ...
        for (const sample of audioTrack.samples) {
            ...
            if (
                lastAudioPts >= 0 &&
                sample.pts - lastAudioPts > GAP_THRESHOLD_MS
            ) {
    ...
    try {
        await invoke("prepare_audio_stream", {
            roomId,
