...
export class FlvDemuxer {
    ...
    // 参考帧率（SPS 不可用时的回退值）
    private _referenceFrameRate = { fixed: true, fps: 30, fps_num: 30, fps_den: 1 }

    // ── 回调 ────────────────────────────────────────────

    onMetaDataArrived: ((meta: Record<string, unknown>) => void) | null = null
    onMediaInfo: ((mediaInfo: MediaInfo) => void) | null = null
    onTrackMetadata: ((type: 'audio' | 'video', meta: AudioMetadata | VideoMetadata) => void) | null = null
    onDataAvailable: ((audioTrack: AudioTrack, videoTrack: VideoTrack) => void) | null = null
    ...
    static probe(data: Uint8Array): {
        ...
        const offset = (data[5] << 24) | (data[6] << 16) | (data[7] << 8) | data[8]
    ...
    feed(chunk: Uint8Array): void {
        ...
        while (this._buffer.length >= PREV_TAG_SIZE_BYTES + TAG_HEADER_BYTES) {
            ...
            const prevTagSize = (this._buffer[0] << 24) | (this._buffer[1] << 16) | (this._buffer[2] << 8) | this._buffer[3]
            // 首个 PreviousTagSize 必须为 0
            if (this._tagPosition === this._dataOffset && prevTagSize !== 0) {
                this._onError(DemuxErrors.FORMAT_ERROR, `首个 PreviousTagSize 应为 0，实际为 ${prevTagSize}`)
            }

            // 读取 tag header（buffer[4..14]）
            const tagType = this._buffer[4]
            const dataSize = ((this._buffer[5] << 16) | (this._buffer[6] << 8) | this._buffer[7]) >>> 0
            ...
            const timestamp =
                ...
                (this._buffer[11] << 24)) >>> 0 // extended upper 8 bits
            ...
            // 提取 tag 数据
            const tagData = this._buffer.subarray(tagDataOffset, tagDataOffset + dataSize)
            ...
            switch (tagType) {
                case 8:
                    ...
                    break
                case 9:
                    this._parseVideoTag(tagData, timestamp, this._tagPosition + PREV_TAG_SIZE_BYTES)
                    break
                case 18:
            ...
            if (nextPrevTagSize !== expectedPrevTagSize) {
                logWarn(`[FlvDemuxer] PreviousTagSize 不匹配: 期望=${expectedPrevTagSize}, 实际=${nextPrevTagSize}`)
            }
    ...
    private _parseScriptTag(data: Uint8Array): void {
        const scriptData = parseScriptData(data.buffer, data.byteOffset, data.byteLength)
        const onMetaData = scriptData['onMetaData'] as Record<string, unknown> | undefined
        ...
        if (onMetaData) {
            ...
            if (typeof onMetaData['duration'] === 'number') {
                this._mediaInfo.duration = Math.floor(onMetaData['duration'] * 1000)
            }
            ...
            // 关键帧索引
            if (onMetaData['keyframes'] && typeof onMetaData['keyframes'] === 'object') {
                const kf = onMetaData['keyframes'] as Record<string, unknown>
                if (Array.isArray(kf['times']) && Array.isArray(kf['filepositions'])) {
    ...
    private _parseAACAudioSpecificConfig(array: Uint8Array): void {
        if (array.length < 2) {
            this._onError(DemuxErrors.FORMAT_ERROR, 'AudioSpecificConfig 数据不足')
            return
        }
        ...
        if (samplingIndex >= MPEG_SAMPLING_RATES.length) {
            this._onError(DemuxErrors.FORMAT_ERROR, `无效的 AAC 采样率索引: ${samplingIndex}`)
            return
        }
        ...
        config[1] = ((samplingIndex & 0x01) << 7) | ((channelConfig & 0x0f) << 3)
        if (audioObjectType === 5) {
            // HE-AAC SBR
    ...
    private _parseVideoTag(data: Uint8Array, timestamp: number, tagPosition: number): void {
        ...
        if (avcPacketType === 0) {
            ...
            // NAL unit data
            this._parseAVCVideoData(avcData, frameType, timestamp, cts, tagPosition)
        }
    ...
    private _parseAVCDecoderConfigurationRecord(data: Uint8Array): void {
        if (data.length < 7) {
            this._onError(DemuxErrors.FORMAT_ERROR, 'AVCDecoderConfigurationRecord 数据不足')
            return
        }

        const version = data[0]
        if (version !== 1) {
            this._onError(DemuxErrors.FORMAT_ERROR, `不支持的 AVCC 版本: ${version}`)
            return
        }
        ...
        if (this._naluLengthSize !== 3 && this._naluLengthSize !== 4) {
            this._onError(DemuxErrors.FORMAT_ERROR, `无效的 NALU 长度大小: ${this._naluLengthSize}`)
            return
        }
        ...
        let codecWidth = spsInfo?.codec_size.width ?? this._mediaInfo.width ?? 0
        let codecHeight = spsInfo?.codec_size.height ?? this._mediaInfo.height ?? 0
        ...
        if (this._mediaInfo.height == null) this._mediaInfo.height = codecHeight
    ...
    private _parseAVCVideoData(
        ...
        while (offset + this._naluLengthSize <= data.length) {
            ...
            if (this._naluLengthSize === 4) {
                naluLength = (data[offset] << 24) | (data[offset + 1] << 16) | (data[offset + 2] << 8) | data[offset + 3]
            } else {
                // naluLengthSize === 3
                naluLength = (data[offset] << 16) | (data[offset + 1] << 8) | data[offset + 2]
            }
    ...
    private _isInitialMetadataDispatched(): boolean {
        ...
        const audioOk = !this._mediaInfo.hasAudio || this._audioMetadataDispatched || this._hasAudioFlagOverrided
        const videoOk = !this._mediaInfo.hasVideo || this._videoMetadataDispatched || this._hasVideoFlagOverrided
