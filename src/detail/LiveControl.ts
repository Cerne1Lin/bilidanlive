...
export function useLiveControl() {
    ...
    function setAudioOnly(val: boolean) {
        ...
        if (val && LiveAudio.isVideoPlaying.value) {
            ...
            !val &&
            LiveAudio.isPlaying.value &&
            !LiveAudio.isVideoPlaying.value
        ) {
    ...
    async function linkToRoom(roomId: number): Promise<boolean> {
        ...
        try {
            ...
            const tasks: Promise<any>[] = [
                invoke<{ code: number; data: RoomInfo | null }>(
                    "get_room_info",
                    { roomId },
                ),
            ];
            ...
            if (infoRes.data) {
                ...
                const loginStatus = await invoke<{ logged_in: boolean }>(
                    "get_login_status",
    ...
    async function fetchHistoryDanmu(roomId: number) {
        try {
            const list = await invoke<HistoryItem[]>("get_history_danmu", {
                roomId,
            });
            for (const h of list) {
