...
export class VideoScheduler {
    ...
    setRenderCallback(cb: (frame: VideoFrame) => void): void {
    ...
    private getClockMs(): number {
        ...
        if (this.videoStartWallMs > 0) {
            return (
                this.videoStartPts + (performance.now() - this.videoStartWallMs)
            );
        }
    ...
    private renderNextFrame(): void {
        if (this.frameQueue.length === 0) {
            ...
            } else if (performance.now() - this.idleSince > IDLE_TIMEOUT_MS) {
                this.stopRafLoop();
            }
            return;
        }
        ...
        const frameInterval =
            this.lastRenderPts > 0
                ? Math.max(
                      16,
                      Math.min(50, this.frameQueue[0].pts - this.lastRenderPts),
                  )
                : 33;
