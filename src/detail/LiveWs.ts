...
interface RoomStatusPayload { online: number }
interface WsErrorPayload { message: string }
...
async function connect(roomId: number) {
    ...
    danmuChannel.onmessage = (payload) => {
        addDanmu({
            id: 0, ts: payload.ts, text: payload.text,
            nickname: payload.nickname, timeline: payload.timeline,
            medal: payload.medal, user: payload.user,
            emoticon: payload.emoticon, type: payload.type, sc: null,
        })
    }
    ...
    scChannel.onmessage = (payload) => {
        addSc({
            ...
            price: payload.price, time: payload.time * 1000,
            ts: payload.ts, medal: payload.medal, user_info: payload.user_info,
        })
    }

    const statusChannel = new Channel<RoomStatusPayload>()
    statusChannel.onmessage = (payload) => { liveOnline.value = payload.online }
    ...
    await invoke('connect_live_room', {
        roomId, danmuChannel, scChannel, statusChannel, errorChannel,
    })
}
...
export default {
    isConnected, liveOnline, isConnecting,
    connectLiveRoom, disconnectLiveRoom,
    getCurrentRoomId, toggleConnect,
    setCurrentRoomId,
} as const
