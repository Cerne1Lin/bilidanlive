<template>
    <div class="container">
        <div class="face-name-label">
            <div class="face"><BiliImg :src="face" :default="img.defaultFace"/></div>
            <div class="name">{{ name }}</div>
            <div class="price">{{ price + 'CNY' }}</div>
        </div>
        <div class="sc-content"><span>{{ text }}</span></div>
    </div>
</template>
...
const scColor = computed(() => {
    if (props.price) {
        ...
        if (p < 50) {
            ...
            return { front: Colors.SC_GREYBLUE_FRONT, back: Colors.SC_GREYBLUE_BACK }
        } else if (p >= 100 && p < 500) {
            return { front: Colors.SC_YELLOW_FRONT, back: Colors.SC_YELLOW_BACK }
        } else if (p >= 500 && p < 1000) {
            return { front: Colors.SC_ORANGE_FRONT, back: Colors.SC_ORANGE_BACK }
        } else {
